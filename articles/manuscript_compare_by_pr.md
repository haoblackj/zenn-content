...
自動修正に対応したtextlintルール(句読点修正、空白抜け、全半角など)では、Pull Requestに対するレビューとして修正サジェストを投稿してくれる。
表示された場合はCommit suggestionから、校正したファイルをそのままコミットできる。
iPadなどでちょこちょこと原稿を修正し続けて、表記規則がめちゃめちゃなまま……ということも防げる。
